...
def main():
    ...
    parser.add_argument('--path_baseline_model', type=str, help='path to dir with baseline model', default="yahma/llama-7b-hf") #"decapoda-research/llama-7b-hf"
    parser.add_argument('--path_baseline_long_model', type=str, help='path to dir with baseline long-model', default="jmvcoelho/t5-base-msmarco-squad-query-generation-longp-v2")
    ...
    parser.add_argument('--temperature_list', nargs='+', help='list of models temperatures to run', default=[1.0, 2.0, 4.0, 10.0]) #default=[1.0, 2.0, 4.0, 10.0]

    # Output options and directory
    parser.add_argument('--return_sequences', type=int, help='number of sequences to return', default=10)
    ...
    model_params = [{'base_model' : args.path_baseline_model, 'max_tokens' : 300}] 
    names = ['llama-7b-prompt']
    ...
    for model_p, name in zip(model_params, names):
        queries_expanded = {}

        for query_id in queries:
            queries_expanded[query_id] = {"0": queries[query_id]}

        #model = T5QueryGenerator(base_model=model_p['base_model'], max_tokens=model_p['max_tokens'], device=device)
        model = LLaMAQueryGenerator(base_model=model_p['base_model'], max_tokens=model_p['max_tokens'], device=device)
        ...
        output_dir = f'{args.queries[:-5]}_{name}'
